    ...
    static let json = #"""
    {
      "9c1f4a2e-7b30-4c58-9a11-2d6e83f0b415": {
        ...
        "messages": [
          ...
          {"role": "agent", "text": "It does now — it did not. The first version read the write log once and rebuilt from that snapshot, so a write that landed mid-rebuild was dropped until the next pass. It reads the log position before and after and re-runs any shard whose position moved; the test that catches the old behaviour is in test_reindex_concurrent.py, which drives two concurrent writers into one shard while the rebuild walks it and asserts the doc count once rather than per pass. The failing run before the fix reported shard 17 at 1204 documents against a corpus of 1205, which is the single dropped write, and the run after it reports 1205 on every one of the three cases. I also checked the path nobody asked about: a write that lands AFTER the walk has passed its shard, which is the case a position compare and not a coun…", "ts": "2027-01-15T07:51:00.000Z"},
